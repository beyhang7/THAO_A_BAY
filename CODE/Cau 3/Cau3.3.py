...
# Thực hiện PCA để giảm số chiều về 2
pca = PCA(n_components=2)
X_pca = pca.fit_transform(X_scaled)

#Vẽ đồ thị phân cụm trên mặt phẳng 2D
plt.figure(figsize=(10, 5))
for cluster in range(optimal_k):
    plt.scatter(X_pca[merged_dfx['Cluster'] == cluster, 0], 
                X_pca[merged_dfx['Cluster'] == cluster, 1],
                label=cluster_names[cluster]) 
plt.title('Phân cụm cầu thủ trên mặt phẳng 2D')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
plt.grid(True)
